<template>
  <div class="user-container">
    <div class="title-box">
      ...
      <div class="title-info">Payment History</div>
    </div>
    ...
    <div class="pay-ment-container">
      <el-table :data="tableData" style="width: 100%" height="100%">
        <el-table-column label="From" width="130" fixed>
        ...
        <el-table-column
          ...
          width="200"
        />
        <el-table-column prop="orderNum" label="Transaction ID" width="200" />
        <el-table-column prop="userId" label="User ID" width="210" />
        <el-table-column label="Paytype" width="150">
          <template v-slot="scope">
            {{ scope.row.orderModel ? scope.row.orderModel : "-" }}
          </template>
        </el-table-column>
        <el-table-column
          ...
          width="180"
        />
        <el-table-column prop="payType" label="Payment type" width="150" />
        <el-table-column prop="orderAmount" label="Fiat amount" width="150" />
        <el-table-column
          prop="orderCurrency"
          label="Fiat currency"
          width="150"
        />
        <el-table-column label="Fiat currency rate" width="150">
          <template v-slot="scope">
            {{ scope.row.faitExchangeRate ? scope.row.faitExchangeRate : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Crypto amount" width="150">
          <template v-slot="scope">
            {{ scope.row.cryptoVolume ? scope.row.cryptoVolume : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Crypto currency" width="150">
          <template v-slot="scope">
            {{ scope.row.cryptoCurrency ? scope.row.cryptoCurrency : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Currency rate" width="150">
          <template v-slot="scope">
            {{ scope.row.faitExchangeRate ? scope.row.faitExchangeRate : "-" }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="Crypto actual payment amount" width="240">
          <template v-slot="scope">
            {{ scope.row.payCryptoVolume ? scope.row.payCryptoVolume : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Currency rate（payment）" width="200">
          <template v-slot="scope">
            {{ scope.row.payCryptoExchangeRate ? scope.row.payCryptoExchangeRate : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Settlement amount" width="150">
          <template v-slot="scope">
            {{ scope.row.accountAmount ? scope.row.accountAmount : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Settlement Currency" width="150">
          <template v-slot="scope">
            {{ scope.row.settlementCurrency ? scope.row.settlementCurrency : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Settlement currency rate" width="200">
          <template v-slot="scope">
            {{ scope.row.settlementRate ? scope.row.settlementRate : "-" }}
          </template>
        </el-table-column>
        <el-table-column label="Wallet address" width="200">
          <template v-slot="scope">
            {{ scope.row.address ? scope.row.address : "-" }}
          </template>
        </el-table-column>
        <el-table-column prop="orderStatus" label="Order status" width="150" />
        <el-table-column label="Failed reason" width="200">
          <template v-slot="scope">
...
import { UseTransactionPage, UseTransactionPageDownload } from "@/api/api";
const stateData = reactive({
  ...
  items: [
    ...
    { value: "DELAY_SUCCESS", text: "DELAY_SUCCESS" },
    { value: "DELAY_FAILED", text: "DELAY_FAILED" },
    { value: "ORDER_EXCEPTION", text: "ORDER_EXCEPTION" },
  ],
  selectedItem: "All",
  ymdStr1: "",
  ymdStr2: "",
...
const getData = async (status) => {
  ...
  let res = await UseTransactionPage(params);
  if (res.data.code === "0") {
...
const exportXlsx = async () => {
  ...
  let res = await UseTransactionPageDownload(params);
  if (res) {
    var date1 = new Date();
    var year1 = date1.getFullYear();
    var month1 = ("0" + (date1.getMonth() + 1)).slice(-2);
