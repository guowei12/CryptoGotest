    ...
    <div class="pay-box-num" v-if="!mechShow">
      ...
      <div class="pay-box-con">
        <div class="pay-box-con-left">
          <div class="pay-box-con-left-title">{{ bankAccountName }}</div>
        </div>
      ...
      <div class="pay-box-amount">
        ...
        <div v-if="currencyResponseList?.scale == 0">
          ...
          <div class="field-wrong" v-if="digit && digit < currencyResponseList?.faitMin">The minimum transaction amount
            is
            {{ currencyResponseList?.faitMin }} {{ currencyResponseList?.faitCurrency }}.</div>
          <div class="field-wrong" v-if="digit && digit > currencyResponseList?.faitMax">The maximum transaction amount
            is
            {{ currencyResponseList?.faitMax }} {{ currencyResponseList?.faitCurrency }}.</div>
          <div class='payment-btm'>
            The system will automatically convert Crypto to {{ faitCurrency }} and process with the payment
          </div>
          <van-number-keyboard @input="onInputKeyd" theme="custom" extra-key="." close-button-text="pay" :show="show"
            :maxlength="16" @close="pustPay" @delete="onDeleted" />
        </div>
        <!-- 允许输入正整数 小数，调起纯数字键盘 -->
        ...
        <div v-if="currencyResponseList?.scale > 0">
         ...
         <span class="custom" id="cursor"></span>
          ...
          <div class="field-wrong" v-if="number && number > currencyResponseList?.faitMax">The maximum transaction
            amount
            is
            {{ currencyResponseList?.faitMax }} {{ currencyResponseList?.faitCurrency }}.</div>
          <div class='payment-btm'>
  ...
  setup() {
    ...
    const infoMethods = {
      ...
      async getCurrency(currency: any){
        let res = await getFaitLimit(currency)
        if(res.data.code == 0){
          data.currencyResponseList = res.data.model
        }else{
          proxy.$failToast('fait error', 'failToast', 3000)
          return
        }
      ...
      async pustPay() {
        ...
        let params = {
          ...
          redirectURL: 'https://localhost:3000/loading'//window.location.href
        }
    ...
    onMounted(async () => {
      ...
      await infoMethods.getCurrency( data.faitCurrency )
      // } else {
      //   data.mechShow = false
      // }
